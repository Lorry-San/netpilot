...
3. Use `/help`, `/status`, `/agents` or `/iperf <ip> [port] [threads] [duration] [-R]` in a private chat or an allowed group.

The Bot registers its command menu automatically when it starts. Unbound users receive an unauthorized response in private chat, except that `/bind` remains available. Only the target is required for `/iperf`; the defaults are port `5201`, one stream and 10 seconds. Without `-R`, the Bot asks for upload or download after Agent selection. Explicit reverse mode accepts both `/iperf IP -R` and `/iperf -R IP` and starts the download test directly. The Bot presents the online Agents available to the effective NetPilot user as an authenticated, paginated multi-select keyboard. Callback buttons are tied to the requesting Telegram ID and authorization is checked again on every click. Every Bot response, test result and chart document replies to the command message that started the interaction. Selected Agents run strictly one at a time because a standard iperf3 server accepts only one client; the next Agent is dispatched only after the current task completes. Web Agent permissions and busy/offline checks still apply.
