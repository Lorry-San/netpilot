...
3. Use `/help`, `/status`, `/agents`, `/iperf`, `/iperf <ip> [port] [threads] [duration] [-R]`, `/nexttrace [options] <target>`, `/rei`, or `/ren` in a private chat or an allowed group.
...
The running message shows the current Agent, per-test progress, output-line count and elapsed batch time, with authenticated refresh and stop controls. Progress edits are limited to once every five seconds; critical results honor Telegram `retry_after` rate limits, and the progress message is removed after completion. Final iperf results distinguish the last interval rate from the receiver average. Charts exclude sender/receiver summary rows and include labelled X/Y axes, units, markers and measured values. `/rei` and `/ren` immediately repeat the most recent iperf or NextTrace parameters and Agents for that Telegram user and chat, with the configuration persisted in SQLite. Long raw output is tail-truncated to stay within Telegram message limits.
