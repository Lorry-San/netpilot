...
- In-place Agent updater for native services and the standard Docker Agent, with checksum verification and rollback
...
Administrators can open **Agent**, select **Update command** for a node and run the generated command as `root` on that Agent host. Updating is allowed while an Agent is online, but is blocked while it is running an iperf test. The update does not rotate or expose the Agent token.
...
For a native installation, the existing `/etc/netpilot-agent/env` file is not modified. For Docker, the updater preserves the `NETPILOT_*` environment variables and restart policy from the standard container. Custom Docker networks, mounts, capabilities or other manually added `docker run` options are outside the automatic updater's scope.
