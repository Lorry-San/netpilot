...
- Apple 风格 Liquid Glass 界面：青绿主题、悬浮圆角侧栏、玻璃材质面板、内置背景图，明暗双模式跟随系统
- 全站可中断弹簧动画：页面切换、导航胶囊、对话框、指标数字与速率曲线新点均带非线性动效，并支持减弱动态效果降级
...
3. 在私聊或允许的群组中使用 `/help`、`/status`、`/agents`、`/iperf`、`/iperf <IP> [端口] [线程] [时长] [-R]`、`/nexttrace [参数] <目标>`、`/rei` 或 `/ren`。
...
运行中的消息会显示当前 Agent、单次测试进度、输出行数和批次已用时间，并提供鉴权后的刷新和停止按钮。进度最多每 5 秒更新一次；遇到 Telegram 限流时，关键结果会按 API 返回的等待时间自动重试，测试完成后进度消息会被删除。最终结果包含末速、receiver 平均速度、每次测试及总耗时、Telegram 可折叠原始输出，以及以文件发送的 PNG 曲线图。曲线只绘制有效区间数据，过滤 sender/receiver 汇总行，并包含 X/Y 轴、单位、数据点和数值。`/rei` 和 `/ren` 会从 SQLite 读取同一聊天中该用户最近一次 iperf 或 NextTrace 的参数及 Agent 并立即重测。过长的原始输出会保留尾部以满足 Telegram 消息长度限制。
