...
test('security invariants, roles and Agent installation lock', async (t) => {
  ...
  const privateTmpAgentUpdate = await request(base, `/api/admin/agents/${agentID}/update`, { method: 'POST', body: '{}' }, session);
  assert.equal(privateTmpAgentUpdate.response.status, 409);
  assert.match(privateTmpAgentUpdate.body.error, /v0\.1\.19/);
  database.prepare("UPDATE agents SET version = 'v0.1.19' WHERE id = ?").run(agentID);
  ...
  assert.equal(autoUpdate.body.update.oldVersion, 'v0.1.19');
  ...
  socket.send(JSON.stringify({ type: 'agent.update.started', taskId: updateStart.taskId, payload: { oldVersion: 'v0.1.19' } }));
  socket.send(JSON.stringify({ type: 'agent.update.output', taskId: updateStart.taskId, payload: { line: '>>> upgrading v0.1.19 -> v9.9.9' } }));
  socket.send(JSON.stringify({ type: 'agent.update.done', taskId: updateStart.taskId, payload: { exitCode: 0, oldVersion: 'v0.1.19', newVersion: 'v9.9.9' } }));
  const updateDone = await updateDoneWait;
  assert.equal(updateDone.payload.oldVersion, 'v0.1.19');
  ...
  assert.equal(version.body.current, '0.1.19');
